...
def periodsSplineRiskFreeInterestRate(options, date):
    ...
    for p in periods.keys():
        ...
        # 此处使用SHIBOR来插值
        cs = interpolate.CubicSpline(period, shibor_values)
        sh = cs(tmp)
        shibor[p] = sh/100.0
    return shibor
    ...
    if put.empty:
        ...
        try:
            if len(putAll.loc[FF_idx, 'CLOSE']) > 1:
                put.loc[put.index[-1], 'CLOSE'] = (putAll.loc[FF_idx, 'CLOSE'].values[1] + callAll.loc[FF_idx, 'CLOSE'].values[0]) / 2.0
...
ivix_data = true_ivix[u'收盘价(元)'].tolist()
...
line.add_yaxis("手动计算", ivix, markline_opts=opts.MarkLineOpts(data=[opts.MarkLineItem(type_="max"), opts.MarkLineItem(type_="average")]))
